...
static int madera_map_irq(struct madera *madera, int irq)
{
	...
	printk(KERN_ALERT "madera_map_irq\n");
	if (!madera->irq_dev)
		return -ENOENT;
			...
			irq_handler_t handler, void *data)
{
	...
	printk(KERN_ALERT "madera_request_irq\n");
	if (irq < 0)
		return irq;
...
void madera_free_irq(struct madera *madera, int irq, void *data)
{
	...
	printk(KERN_ALERT "madera_free_irq\n");
	if (irq < 0)
		return;
...
int madera_set_irq_wake(struct madera *madera, int irq, int on)
{
	...
	printk(KERN_ALERT "madera_set_irq_wake\n");
	if (irq < 0)
		return irq;
...
static int madera_suspend_noirq(struct device *dev)
{
	...
	printk(KERN_ALERT "madera_suspend_noirq\n");
...
static int madera_suspend(struct device *dev)
{
	...
	printk(KERN_ALERT "madera_suspend\n");
...
static int madera_resume_noirq(struct device *dev)
{
	...
	printk(KERN_ALERT "madera_resume_noirq\n");
...
static int madera_resume(struct device *dev)
{
	...
	printk(KERN_ALERT "madera_resume\n");
...
static irqreturn_t madera_boot_done(int irq, void *data)
{
	...
	printk(KERN_ALERT "madera_boot_done\n");
...
static int madera_irq_remove(struct platform_device *pdev)
{
	...
	printk(KERN_ALERT "madera_irq_remove\n");
	/*
